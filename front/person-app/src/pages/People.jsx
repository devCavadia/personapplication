...
const People = () => {
  ...
  const [stdNumberFieldDisabled, setStdNumberFieldDisabled] = useState(true);
  const [avgMarkFieldDisabled, setAvgMarkFieldDisabled] = useState(true);
  ...
  const studentNumberFieldRef = useRef(null);
  const averageMarkFieldRef = useRef(null);
  ...
  const recoverPerson = (person) =>{
    ...
    if (person.salary!=null){ setSalary(person.salary); setTypeRecord('PROFESOR'); }
    if (person.studentNumber!=null && person.averageMark!=null ){ 
      setStudentNumber(person.studentNumber); 
      setAverageMark(person.averageMark); 
      setTypeRecord('ESTUDIANTE'); 
    }
    changeShowTable();
  ...
  const validate = () => {  
    ...
    if (typeRecord == "ESTUDIANTE"){
      if (name=="" || emailAddress=="" || studentNumber==0 || averageMark==0){
        setValidateForm('block');
        setBorder('1px solid rgb(219, 45, 45)');
        setStudBorder(true);
        return false;
    ...
    if (peopleList.find(person => person.name==name.trim())!=null && peopleList.find(person => person.emailAddress==emailAddress.trim())!=null){
      setMessage('block');
      return false;
    }
  ...
  const updatePerson = () => {
    if (validate()){
      ...
      if (confirmation){
        if (typeRecord == "SOLO PERSONA"){
          axios.put("http://129.153.211.34:8080/api/person/upd",
            {
              idPerson: idPerson,
              name: name,
        ...
        }else if (typeRecord == "ESTUDIANTE"){
          axios.put("http://129.153.211.34:8080/api/student/upd",
            {
              idPerson: idPerson,
              name: name,
              emailAddress: emailAddress,
      ...
      setStdNumberFieldDisabled(true);
      setAvgMarkFieldDisabled(true);
  ...
  const enableDisableFields = () =>{
    if (buttonEdit =="Editar información"){
      ...
      setStdNumberFieldDisabled(false);
      setAvgMarkFieldDisabled(false);
  ...
  const showHideFiels = () =>{
    if (typeRecord=='SOLO PERSONA'){
      additInfoRef.current.style.display = 'none';
      salaryFieldRef.current.style.display = 'none';
      studentNumberFieldRef.current.style.display = 'none';
      averageMarkFieldRef.current.style.display = 'none';
    }else if (typeRecord=='PROFESOR'){
      ...
      additInfoRef.current.style.display = 'block';
      salaryFieldRef.current.style.display = 'none';
      studentNumberFieldRef.current.style.display = 'block';
      averageMarkFieldRef.current.style.display = 'block';
    }
  }
    ...
    <div className='d-flex'>
      ...
      <div style={ showTable ? { display: 'none' } : {width:'80%', padding: "0 1%", display: 'block'} }>
        ...
        <form onSubmit={ (e)=> handlerSubmit(e) } className="row g-2">
          ...
          <div className="col-md-4" ref={studentNumberFieldRef}>
            <label htmlFor="studentNumber" className="form-label">N° de estudiante<span style={{color: 'rgb(219, 45, 45)'}}>*</span></label>
            <input type="number" className="form-control" id="studentNumber" value={studentNumber} disabled={stdNumberFieldDisabled} style={ studBorder ? {border: `${border}`} : {border: ''}}
              onChange={(e)=>{setStudentNumber(e.target.value); setBorder(''); setStudBorder(false); setValidateForm('none'); setMessage('none');}} />
          </div>
          <div className="col-md-4" ref={averageMarkFieldRef}>
            <label htmlFor="averageMark" className="form-label">Promedio<span style={{color: 'rgb(219, 45, 45)'}}>*</span></label>
            <input type="number" className="form-control" id="averageMark" value={averageMark} disabled={avgMarkFieldDisabled} style={ studBorder ? {border: `${border}`} : {border: ''}}
              onChange={(e)=>{setAverageMark(e.target.value); setBorder(''); setStudBorder(false); setValidateForm('none'); setMessage('none');}}/>
          </div>
      ...
      <div style={ showTable ? {height: 630, width:'80%', textAlign: 'center', padding: "0 1%", display: 'block', overflowY: 'scroll'} : {display: 'none'} }>
        <h2 style={{paddingTop: 50, paddingBottom: 20}}>Listado de Personas</h2>
        <div style={{marginBottom: '1%',display: 'flex', justifyContent: 'left'}}>
          ...
          <input style={{width: '50%', marginLeft:"10%", borderRadius: '5px', border: '1px solid gray'}} 
              type="text" placeholder='Buscar persona por N°, nombre, email, etc.' 
              value={search}
