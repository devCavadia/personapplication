...
const Student = () => {
  ...
  const getPeople = () =>{
    axios.get("http://129.153.211.34:8080/api/student/all")
    .then((res) => {
  ...
  const validate = () => {  
    ...
    if (peopleList.find(person => person.name==name.trim())!=null && peopleList.find(person => person.emailAddress==emailAddress.trim())!=null
      && peopleList.find(person => person.studentNumber==studentNumber)!=null && peopleList.find(person => person.averageMark==averageMark)!=null){
      setMessage('block');
      return false;
    }
  ...
  return (
    <div className='d-flex'>
      ...
      <div style={ showTable ? {height: 630, width:'80%', textAlign: 'center', padding: "0 1%", display: 'block', overflowY: 'scroll'} : {display: 'none'} }>
        <h2 style={{paddingTop: 50, paddingBottom: 20}}>Listado de Estudiantes</h2>
        <div style={{marginBottom: '1%',display: 'flex', justifyContent: 'left'}}>
          ...
          <input style={{width: '50%', marginLeft:"10%", borderRadius: '5px', border: '1px solid gray'}} 
              type="text" placeholder='Buscar estudiante por N°, nombre, email, etc.' 
              value={search}
